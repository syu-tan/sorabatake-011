...
from utils.mae import MaskedAutoencoderViT
...
def save_geotiff(image, output_path: str, meta: dict):
    ...
    with rasterio.open(output_path, "w", **meta) as dest:
        for i in range(image.shape[0]): # channels, height, width
        #     # print(image.shape, i+1)
            dest.write(image[i,], 1)
...
def save_imgs(rec_img, mask_img, mean, std, output_dir, meta_data):
    ...
    for t in range(rec_img.shape[1]):
        ...
        # print(rec_img_t.shape) # channels, height, width
...
def main(data_files: List[str], yaml_file_path: str, checkpoint: str, output_dir: str,
    ...
    if rgb_outputs:
        ...
        # print(rec_imgs_full.shape, mask_imgs_full.shape)
